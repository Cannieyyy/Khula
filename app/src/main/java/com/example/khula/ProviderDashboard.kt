...
class ProviderDashboard : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Overview TextViews
        val tvNewRequests = findViewById<TextView>(R.id.tvNewRequests)
        val tvIncoming = findViewById<TextView>(R.id.tvIncoming)
        ...
        // Load provider overview
        lifecycleScope.launch {

            try {

                val response =
                    RetrofitClient.api.getProviderOverview()
                ...
                if (response.isSuccessful) {
                    ...
                    tvNewRequests.text =
                        overview?.newRequests?.toString() ?: "0"

                    tvIncoming.text =
                        overview?.incoming?.toString() ?: "0"

                    tvCompleted.text =
                        overview?.completed?.toString() ?: "0"
                    ...
                    Toast.makeText(
                        this@ProviderDashboard,
                        "Could not load overview: ${response.code()}",
                        Toast.LENGTH_LONG
                    ).show()
                }
        ...
        for (booking in bookings) {
            ...
            requestCard.setOnClickListener {
                ...
                val intent = android.content.Intent(
                    this,
                    ProviderRequestDetails::class.java
                )
